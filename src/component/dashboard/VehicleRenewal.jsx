// component/dashboard/VehicleRenewal.jsx
...
function VehicleRenewal() {
  ...
  const fetchRenewalReminders = async () => {
    try {
      ...
      // Filter vehicles with renewal due in next 30 days
      const renewalVehicles = response.data.filter(vehicle => 
        new Date(vehicle.renewalDate) <= thirtyDaysFromNow && 
        new Date(vehicle.renewalDate) >= now
      );
      
      // Sort by renewal date (earliest first)
      renewalVehicles.sort((a, b) => new Date(a.renewalDate) - new Date(b.renewalDate));
      
      setVehicles(renewalVehicles);
    } catch (err) {
      console.error("Error fetching renewal reminders:", err);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="reminders-container">
      <div className="reminders-header">
        ...
        <p className="text-muted">Vehicles requiring renewal in the next 30 days</p>
      </div>
      ...
      {vehicles.length === 0 ? (
        <Card className="no-reminders-card">
          <Card.Body className="text-center py-5">
            ...
            <h5 className="text-muted">No Renewal Reminders</h5>
            <p className="text-muted">All vehicle renewals are up to date.</p>
          </Card.Body>
        </Card>
      ) : (
        <>
                ...
                <Row>
                  <Col md={4}>
                    <div className="summary-item text-center">
                      <h3 className="text-danger">{vehicles.filter(v => getDaysUntilRenewal(v.renewalDate) <= 7).length}</h3>
                      <p className="text-muted mb-0">Urgent (Within 7 days)</p>
                    </div>
                  </Col>
                  <Col md={4}>
                    <div className="summary-item text-center">
                      <h3 className="text-warning">{vehicles.filter(v => getDaysUntilRenewal(v.renewalDate) > 7 && getDaysUntilRenewal(v.renewalDate) <= 14).length}</h3>
                      <p className="text-muted mb-0">Soon (8-14 days)</p>
                    </div>
                  </Col>
                  <Col md={4}>
                    <div className="summary-item text-center">
                      <h3 className="text-info">{vehicles.filter(v => getDaysUntilRenewal(v.renewalDate) > 14).length}</h3>
            ...
            {vehicles.map((vehicle) => {
              ...
              return (
                <Col key={vehicle._id} xl={6} lg={6} md={12}>
                  <Card className="reminder-card renewal-reminder">
                    <Card.Body>
                      ...
                      <div className="reminder-details">
                        <div className="detail-row">
                          ...
                          <span>Renewal Due: <strong>{new Date(vehicle.renewalDate).toLocaleDateString()}</strong></span>
                        </div>
                        <div className="detail-row">
                          <i className="bi bi-clock me-2 text-primary"></i>
                          <span>Due in: <strong>{daysUntilRenewal} day{daysUntilRenewal !== 1 ? 's' : ''}</strong></span>
                        </div>
                        {vehicle.lastRenewed && (
                          <div className="detail-row">
                            <i className="bi bi-check-circle me-2 text-muted"></i>
                            <span>Last Renewed: {new Date(vehicle.lastRenewed).toLocaleDateString()}</span>
                          </div>
                        )}
                      </div>
                      ...
                      <div className="reminder-actions mt-3">
                        <small className="text-muted">
                          ...
                          Visit RTO or online portal for renewal process
                        </small>
                      </div>
                    </Card.Body>
                  </Card>
